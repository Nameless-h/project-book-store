...
public class statistic_employee extends JPanel implements ActionListener,KeyListener{
    ...
    private String[] columnNames = {"ID","Tên nhân viên","Chức vụ","Doanh thu"};
    ...
    public statistic_employee() {
        ...
        // get table data
        nv = new thongKeSachDAO();
        listNV = nv.selectThongKeNV();
    ...
    private void setDataToTable(String name , statisticTable t) {
        ...
        for (nhanvien i : listNV) {
            if( String.valueOf(i.getMa()).contains(name) || 
                ...
                i.getChucvu().toLowerCase().contains(name.toLowerCase())
            ) { 
                t.addRow(new Object[]{
                    i.getMa(),
                    i.getTen(),
